/**
 * @file ply_to_xyz_transframer_interface.hpp
 *
...
/*******************************************************************************
*/
TransframerInterface::TransframerInterface() : Node("ply_to_xyz_transframer_node"),
  ...
  has_functionality_been_delivered_successfully_(false)
{
  ...
  // Declare params to be passed to node `ply_to_xyz_transframer` via `execute()`
  declare_params();
  ...
  if (state_t0_.load() != state_t1_.load())
  {
    ...
    if (transition_table_[state_t0_.load()][state_t1_.load()] == true)
    {
      ...
      if (state_t1_.load() == PLAY)
      {
        ...
        thread_start("ply_to_xyz_transframer_thread");
      }
      // -------------------------------------
      if (state_t1_.load() == PAUSE)
      {
        RCLCPP_INFO(this->get_logger(), "Triggering execution PAUSE");
        thread_stop("ply_to_xyz_transframer_thread");
      }
      // -------------------------------------
 ...
 * values.  If declared downstream (ply_to_xyz_transframer [without the word
...
TransframerInterface::declare_params()
{
  ...
  this->declare_parameter("input_file_pattern", ".*\\.ply");
